...
export class OrderService {
  ...
  async create(createOrderDto: CreateOrderDto, user: any) {
    ...
    try {
      ...
      const { hash_password, username, salt, roles, ...safeCustomer } = customer;
      if (!customer) throw new NotFoundException(`User with ID ${user.id} not found`);
  ...
  async generateInvoice(data: CreateInvoiceDto) {
    ...
    try {
      const order = await queryRunner.manager.findOneBy(Order, { id: data.orderId });
      if (!order) {
